...
import {
  authenticate,
  checkExtAuthenticationState,
  initExtAuthentication,
} from "./AuthUtils";
...
async function checkSigning() {
  try {
    ...
    if (!provider) {
      ...
      let did = decrypt(
        localStorage.getItem(STORAGE_KEYS.userDID),
        loggedInState
      );
    ...
    if (signingInfoSet.length < 1) {
      signingInfoSet = JSON.parse(
        decrypt(
          localStorage.getItem(STORAGE_KEYS.signingInfoSet),
          loggedInState
        )
      );
    ...
    switch (request.task) {
      ...
      case TASKS.GET_IDENTITY: {
        ...
        try {
          ...
          let encryptedSigningInfo = localStorage.getItem(
            STORAGE_KEYS.signingInfoSet
          );
          if (encryptedDID) {
...
function changePassword(oldPassword: string, newPassword: string): boolean {
  if (login(oldPassword)) {
    ...
    if (changed) {
      ...
      if (encryptedDID) {
        let didRecrypted = encrypt(
          decrypt(encryptedDID, oldPassword),
          newPassword
        );
...
async function changeDID(did: string): Promise<string> {
  try {
    ...
    let newProvider = await Provider.getProvider(did, undefined, [
      keyResolve2018,
    ]);
    ...
    signingInfoSet = [];
    let encryptedSigningInfo = encrypt(
      JSON.stringify(signingInfoSet),
      loggedInState
    );
  ...
  try {
    ...
    if (Array.isArray(settings?.did_resolver)) {
      localStorage.setItem(
        STORAGE_KEYS.crypto_suit,
        settings?.did_resolver[0]?.cryptoSuite
      );
    }
    ...
    if (settings?.signing_params?.private_key)
      addKey(settings.signing_params.private_key);
